...
// always carries a port. Before this was handled, every audit event was
// stored with a NULL ip_address even though the value had been collected
// correctly — mirrors the identical regression already covered for sessions
// in internal/platform/postgres/repository/session.
func TestParseIP(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:  "unparseable value stores NULL rather than failing the request",
