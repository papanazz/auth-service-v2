...
- **Email verification is entirely unbuilt** — see `docs/register.md`
  Gaps. A consequence for login specifically: `EmailVerifiedAt` is never
  checked, so `errs.ErrEmailNotVerified` (defined, mapped to 403) is
