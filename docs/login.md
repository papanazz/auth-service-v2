...
- **`last_login_at` is written outside the login transaction, best-effort.**
  ...
  pair, since it isn't critical to authentication succeeding — holding
  the transaction open for a non-critical write, or worse, rolling back
  a real login because this write failed, would be the wrong trade. Sits
  alongside audit publishing after commit, with the same
  errors-are-swallowed treatment; a login is not allowed to fail because
  either of these did.
...
- `UpdateLastLoginAt` is called once with the correct user ID on
  success, and not at all when login does not succeed (locked account);
  a failure recording it does not fail an otherwise-successful login
...
- registered a fresh account (`last_login_at` `NULL`), logged in once
  (`last_login_at` populated), logged in again from a second device
  (`last_login_at` advances to the later timestamp)
